      ...
      try {
        ...
        if (res.ok) {
          ...
          setForm({
            ...
            hours: data.hours || form.hours,
            accentColor: data.accentColor || "#6B4FE0",
            fontPair: data.fontPair || "manrope-source",
  ...
  if (loading) {
    return (
      <div className="p-6 lg:p-8 max-w-7xl mx-auto flex items-center justify-center min-h-[50vh]">
        <Loader2 className="w-8 h-8 text-[#6B4FE0] animate-spin" />
      </div>
    );
  }
    ...
    <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold text-[#1B1530]">
          Edit: {form.businessName || "Untitled site"}
        </h1>
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-[#ECE6DE] px-6 py-4 flex items-center justify-between z-40">
        <button
          onClick={() => router.push(`/dashboard/sites/${params.id}`)}
        ...
        <button
          onClick={handleUpdate}
          disabled={saving}
          className="px-6 py-2.5 rounded-xl text-white font-semibold bg-[#6B4FE0] hover:bg-[#5a3fcf] transition-colors text-sm disabled:opacity-50 flex items-center gap-2"
        >
          {saving ? (
            <span className="flex items-center gap-2">
              <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
